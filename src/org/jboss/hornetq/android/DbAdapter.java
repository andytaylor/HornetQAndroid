...
public class DbAdapter
{
   ...
   public static final String SETTINGS_HOST = "url";
   ...
   public Settings saveSettings(String username, String password, String host, String port)
   {
      deleteSettings();
      ContentValues cv = new ContentValues();
      putCV(cv, SETTING_USERNAME, username);
      putCV(cv, SETTING_PASSWORD, password);
      putCV(cv, DbAdapter.SETTINGS_HOST, host);
      putCV(cv, SETTING_PORT, port);
      return new Settings(mDb.insert(SETTINGS_TABLE, null, cv), username, password, host, port);
   }
   ...
   {
      Cursor cursor = mDb.query(true, SETTINGS_TABLE, new String[]{SETTING_ID, SETTING_USERNAME, SETTING_PASSWORD, SETTINGS_HOST, SETTING_PORT},
      ...
      if (cursor != null && cursor.moveToNext())
      {
         return new Settings(cursor.getLong(cursor.getColumnIndex(SETTING_ID)), cursor.getString(cursor.getColumnIndex(SETTING_USERNAME)),
               cursor.getString(cursor.getColumnIndex(SETTING_PASSWORD)), cursor.getString(cursor.getColumnIndex(SETTINGS_HOST)),
               cursor.getString(cursor.getColumnIndex(SETTING_PORT)));
      } else
      {
         return null;
      }
